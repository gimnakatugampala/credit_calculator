...
export default function CreditCalculator() {
  ...
  const saveToSupabase = async (data) => {
    if (!supabase) return;
    const operation = { type: 'upsert', userId, data };
    try {
      await supabase
        .from('calculator_data')
        .upsert({
  ...
  const deleteFromSupabase = async () => {
    if (!supabase) return;
    const operation = { type: 'delete', userId };
    try {
      await supabase.from('calculator_data').delete().eq('user_id', userId);
      setSyncStatus('synced');
    } catch (error) {
  ...
  useEffect(() => {
    const handleOnline = () => {
      setIsOnline(true);
      if (supabase && syncQueueRef.current.length > 0) processSyncQueue();
    };
    const handleOffline = () => {
      setIsOnline(false);
      setSyncStatus('offline');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (syncQueueRef.current.length > 0 && isOnline) processSyncQueue();
    }, 30000);
  ...
  const deleteModule = (semesterId, moduleId) => {
    setSemesters(semesters.map(sem => {
      if (sem.id === semesterId) {
        return { ...sem, modules: sem.modules.filter(mod => mod.id !== moduleId) };
      }
      return sem;
    }));
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="relative max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-6 sm:py-8 md:py-12">
        ...
        <div className="mb-6 md:mb-8 text-center relative">
          ...
          {/* RESTART BUTTON — always top-right */}
          <button
            onClick={() => setShowRestartModal(true)}
            className="absolute right-0 top-0 inline-flex items-center gap-2 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-sm font-bold shadow-lg hover:shadow-xl transition-all duration-200 z-10"
          >
            <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5}
                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              ...
              {semesters.some(sem => sem.modules.some(m => hasValidMark(m))) && (
                <div className="space-y-2 md:space-y-3 mb-4 md:mb-6">
                  ...
                  {semesters.map(semester => {
                    ...
                    const semesterWeightedSum = gradedModules.reduce((sum, m) => sum + (parseFloat(m.mark) * (parseInt(m.credits) || 0)), 0);
              ...
              <div className="bg-gradient-to-br from-slate-50 to-indigo-50 rounded-lg md:rounded-xl p-3 md:p-4 border border-slate-200">
                ...
                <div className="space-y-1.5 md:space-y-2">
                  {[
                    ...
                    { color: 'bg-blue-500', label: 'Upper Second Class', range: '60-69%' },
                    { color: 'bg-yellow-500', label: 'Lower Second Class', range: '50-59%' },
                    { color: 'bg-gray-500', label: 'Third Class', range: '40-49%' },
                    { color: 'bg-red-500', label: 'Fail', range: '<40%' },
